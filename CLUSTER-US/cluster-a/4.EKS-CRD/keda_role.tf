...
resource "aws_iam_policy" "keda_policy" {
  name        = format("%s-%s-%s", "keda-policy", data.terraform_remote_state.eks_core.outputs.cluster-name, "${random_string.suffix.result}")
...
module "iam_assumable_role_keda" {
  ...
  role_name   = format("%s-%s-%s", "keda-iam-role", data.terraform_remote_state.eks_core.outputs.cluster-name, "${random_string.suffix.result}")
  create_role = true
  oidc_providers = {
    main = {
  ...
  tags = {
    Name = format("%s-%s-%s", "keda-iam-role", data.terraform_remote_state.eks_core.outputs.cluster-name, "${random_string.suffix.result}")
  }
}

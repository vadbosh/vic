...
module "ingress_nginx_version" {
  source             = "./modules/helm-version"
  chart_name         = var.ingress_internal_config.chart_name
  versions_file_path = "${path.root}/helm_versions.json"
}

resource "helm_release" "internal_ingress_nginx" {
  count            = module.ingress_nginx_version.version != null ? 1 : 0
  name             = var.ingress_internal_config.helm_release_name # !!! internal
  namespace        = var.ingress_internal_config.namespace         # !!! internal
  wait             = true
  create_namespace = true
  chart            = var.ingress_internal_config.chart_name
  repository       = "https://kubernetes.github.io/ingress-nginx"
  version          = module.ingress_nginx_version.version
  timeout          = "300"
...
controller:
  ...
  ingressClass: ${var.ingress_internal_config.class_name} # !!! internal
  ingressClassResource:
    name: ${var.ingress_internal_config.class_name} # !!! internal 
    controllerValue: "k8s.io/${local.ingress_controller.internal_nginx}" # !!! internal
    enabled: true
  resources:
   #limits:
   #  cpu: 900m
  ...
  config:
    ...
    http-snippet: |
      more_set_headers "Server: WL-${local.ingress_2lb_name.internal_nginx}";
        ...
        podAffinityTerm:
          labelSelector:
            ...
            - key: release
              operator: In
              values:
              - ${local.ingress_controller.internal_nginx} # !!! internal
  ...
  service:
    annotations:
      ...
      service.beta.kubernetes.io/aws-load-balancer-name: "${local.ingress_2lb_name.internal_nginx}"
      service.beta.kubernetes.io/aws-load-balancer-subnets: "${join(",", data.terraform_remote_state.network.outputs.private_subnet_ids)}"
      service.beta.kubernetes.io/aws-load-balancer-additional-resource-tags: "Name=${local.ingress_2lb_name.internal_nginx}"

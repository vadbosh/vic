...
module "external_ingress_nginx_shared_version" {
  source             = "./modules/helm-version"
  chart_name         = var.ingress_external_shared_config.chart_name
  versions_file_path = "${path.root}/helm_versions.json"
}

resource "helm_release" "external_ingress_nginx_shared" {
  count            = module.external_ingress_nginx_shared_version.version != null ? 1 : 0
  name             = var.ingress_external_shared_config.helm_release_name
  namespace        = var.ingress_external_shared_config.namespace
  wait             = true
  create_namespace = true
  chart            = var.ingress_external_shared_config.chart_name
  repository       = "https://kubernetes.github.io/ingress-nginx"
  version          = module.external_ingress_nginx_shared_version.version
  timeout          = "300"
...
controller:
  ...
  ingressClass: ${var.ingress_external_shared_config.class_name}
  ingressClassResource:
    name: ${var.ingress_external_shared_config.class_name}
    controllerValue: "k8s.io/${local.ingress_controller.external_nginx_shared}"
    enabled: true
  resources:
    #limits:
    # cpu: 900m
  ...
  config:
    ...
    http-snippet: |
      more_set_headers "Server: WL-${local.ingress_2lb_name.external_nginx_shared}";
        ...
        podAffinityTerm:
          labelSelector:
            ...
            - key: release
              operator: In
              values:
              - ${local.ingress_controller.external_nginx_shared}
  ...
  service:
    annotations:
      service.beta.kubernetes.io/aws-load-balancer-security-groups: "${local.ingress_security_group_ids.external_nginx_shared}"
      service.beta.kubernetes.io/aws-load-balancer-manage-backend-security-group-rules: 'true'
      service.beta.kubernetes.io/aws-load-balancer-scheme: "internet-facing"
      service.beta.kubernetes.io/aws-load-balancer-type: "nlb"
      service.beta.kubernetes.io/aws-load-balancer-name: "${local.ingress_2lb_name.external_nginx_shared}"
      service.beta.kubernetes.io/aws-load-balancer-additional-resource-tags: "Name=${local.ingress_2lb_name.external_nginx_shared}"
